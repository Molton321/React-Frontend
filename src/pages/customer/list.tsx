...
const CustomerListPage: React.FC = () => {
  ...
  const handleAccion = async (accion: string, item: any) => {
    if (accion === 'ver') {
      navigate(`/customer/view/${item.id}`);
    } else if (accion === 'editar') {
      navigate(`/customer/update/${item.id}`);
    } else if (accion === 'eliminar') {
      if (window.confirm(`¿Seguro que deseas eliminar el cliente #${item.id}?`)) {
        const success = await customerService.deleteCustomer(item.id);
        if (success) {
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <AddButton label="Agregar Cliente" to="/customer/create" />
      </div>
      {loading ? (
        <div>Cargando...</div>
      ) : (
